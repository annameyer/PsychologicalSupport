...
namespace PsychologicalSupports.Models
{
    ...
    {
        public static ExcelPackage GenerateExcelFile<T>(IEnumerable<T> ds, List<string> Fio)
        {
            ...
            foreach (T elem in ds)
            {
                    ...
                    {
                        if (prop.ToString().Contains("System.Collections.Generic") ||
                            prop.ToString().Contains("PsychologicalSupports.Models"))
                        {
                            continue;
                        }
                        ...
                        if (j == 0)
                        {
                            ...
                            ws.Cells[1, j + 1].Value = "Учащийся";
                            ws.Cells[1, j + 1].Style.Font.Bold = true;
                            ws.Cells[i, j + 1].Value = Fio[i-2];
                        }
                        else
                        {
